...
- **Гибридная память** — векторное хранилище с RAG + LLM Wiki (compiled knowledge base по паттерну Karpathy) + `smart_search` — единый инструмент, ищущий одновременно в wiki, knowledge base и memory
...
### Гибридная память
- **`smart_search`** — единый инструмент поиска: автоматически ищет в LLM Wiki (курированные страницы) → Knowledge Base (RAG, сырые чанки) → Memory (сохранённые факты), объединяет и ранжирует результаты с source attribution ([Wiki]/[RAG]/[Memory])
